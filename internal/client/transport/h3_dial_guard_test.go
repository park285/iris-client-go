...
func TestH3DialGuardLiteralIPFastPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			guard, err := newTestH3DialGuard(t.Context(), t, tt.baseURL, clock, resolver)
			if err != nil {
...
func TestH3DialGuardConcurrentRefreshUsesStaleAllowsetAndSingleFlight(t *testing.T) {
	...
	guard, err := newTestH3DialGuard(t.Context(), t, "https://iris.test:31001", clock, resolver, WithH3DialGuardTTL(time.Minute))
	if err != nil {
		t.Fatalf("newH3DialGuardForBaseURL() error = %v", err)
	}
...
func TestH3DialGuardRefreshFailureKeepsStaleAllowsetAndWarns(t *testing.T) {
	...
	record := handler.Last()
	if record.level != slog.LevelWarn || record.attrs["host"] != "iris.test" || !errors.Is(testsupport.AssertType[error](t, "record.attrs[err]", record.attrs["err"]), resolveErr) {
...
func TestH3DialGuardFailFastInitialResolve(t *testing.T) {
	...
	_, err := newTestH3DialGuard(t.Context(), t, "https://iris.test:31001", clock, resolver)
...
func TestH3DialGuardLenientInitDeniesUntilTTLRecovery(t *testing.T) {
	...
	if record := handler.Last(); record.level != slog.LevelWarn || record.attrs["host"] != "iris.test" || !errors.Is(testsupport.AssertType[error](t, "record.attrs[err]", record.attrs["err"]), resolveErr) {
...
func TestH3DialGuardSynchronousRefreshHonoursDialContext(t *testing.T) {
	...
	if err := guard(dialCtx, newIP); err == nil {
		t.Fatal("guard(canceled dial) error = nil, want denial instead of blocking on the refresh")
	}
}
