...
var _ ReactionClient = (*APIClient)(nil)

// SendReaction은 room에 typed idempotent reaction 요청을 제출합니다.
func (c *APIClient) SendReaction(ctx context.Context, room int64, req ReactionRequest) (*ReactionResponse, error) {
	if room <= 0 {
	...
	resp, err := c.postStrictJSON[ReactionResponse](ctx, path, req, SecretRoleBotControl)
	if err != nil {
...
func validateReactionIdentity(req ReactionRequest) error {
	...
	if chatLogID == "" {
		return errors.New("chatLogId must not be blank")
	}

	if chatLogID[0] == '0' {
		return errors.New("chatLogId must be a canonical positive integer")
	}
...
func validateReactionOperationShape(req ReactionRequest) error {
	...
	if operationCount == 0 {
		return errors.New("at least one reaction operation is required")
	}

	if len(req.Add) > 0 && (len(req.Follow) > 0 || len(req.Remove) > 0) {
		return errors.New("add cannot be combined with follow or remove")
	}
