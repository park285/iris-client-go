package transport

import (
	"bytes"
	jsonv2 "encoding/json/v2"
	"net/http"
	"net/http/httptest"
	"testing"
)

func TestAPIClientSendImageUsesKnownLengthMultipartRequest(t *testing.T) {
	t.Parallel()

	var sawKnownLength bool

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		sawKnownLength = r.ContentLength > 0 && len(r.TransferEncoding) == 0

		metadata, images := readMultipartReplyRequest(t, r)
		if metadata.Type != msgTypeImage || metadata.Room != testRoom {
			t.Fatalf("metadata = %+v, want image room", metadata)
		}

		if len(images) != 1 || !bytes.Equal(images[0], []byte{0x89, 'P', 'N', 'G'}) {
			t.Fatalf("images = %v, want one png payload", images)
		}

		w.Header().Set("Content-Type", contentTypeJSON)

		if err := jsonv2.MarshalWrite(w, ReplyAcceptedResponse{
			Success:   true,
			Delivery:  testDeliveryQueued,
			RequestID: "stream",
			Room:      testRoom,
			Type:      msgTypeImage,
		}); err != nil {
			t.Errorf("Encode() error = %v", err)
		}
	}))

	defer server.Close()

	c := NewAPIClient(server.URL, "", WithHTTPClient(server.Client()))
	if _, err := c.SendImage(t.Context(), testRoom, []byte{0x89, 'P', 'N', 'G'}); err != nil {
		t.Fatalf("SendImage() error = %v", err)
	}

	if !sawKnownLength {
		t.Fatal("request did not arrive with known content length")
	}
}
